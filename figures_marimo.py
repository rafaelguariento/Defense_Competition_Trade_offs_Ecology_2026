...
def _():
    ...
    return PARAMS_ISO, color_fader, integrate, np, pd, plt
...
@app.cell
def _(PARAMS_ISO, color_fader, np, plt, resolve_path, save_fig):
    def plot_outermost_zngi_colored(zngi_number=4, alpha=2):
        """Figure 3 variant: Panel B with colored coexistence/exclusion zones."""
        c1, c2 = '#1f77b4', '#FFBF00'
        g, f, e_p, c, d, dp = (PARAMS_ISO['g'], PARAMS_ISO['f'], PARAMS_ISO['e_p'],
                                PARAMS_ISO['c'], PARAMS_ISO['d'], PARAMS_ISO['dp'])
        prey_labels_all = ['Prey D', 'Prey C', 'Prey B', 'Prey A']
        prey_colors_all = ['#FFBF00', color_fader(c1, c2, 0.33),
                           color_fader(c1, c2, 0.66), '#1f77b4']
        line_lw, impact_lw = 2.6, 1.2

        # x-values for each prey (sorted ascending: D=0.25, C=0.5, B=0.75, A=1.0)
        ...
        def R_star(x):
            """R-intercept of ZNGI for prey with trait x."""
            return d / (c * g * x)

        # ---- Panel A: same as original -------------------------------------------
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 7))

        R1 = np.linspace(0, 15, 500)
        for ix, x in enumerate(x_vals):
            z = zngi(R1, x)
            if ix == 0:
                lbl = f'{prey_labels_all[ix]}\n(more defended), $x$ = {x:.2f}'
            elif ix == zngi_number - 1:
                lbl = f'{prey_labels_all[ix]}\n(more competitive), $x$ = {x:.2f}'
            else:
                lbl = f'{prey_labels_all[ix]}, $x$ = {x:.2f}'
        ...
        for target in ['Prey A', 'Prey B', 'Prey C', 'Prey D']:
            for h, l in zip(handles, lbls):
                if l.startswith(target):
                    oh.append(h); ol.append(l); break
        ax1.legend(oh, ol, loc='lower right', bbox_to_anchor=(1.02, -0.01),
                   fontsize=15, frameon=True, facecolor='white', edgecolor='none',
                   framealpha=0.9, borderpad=0.35, labelspacing=0.25,
                   handlelength=1.2, handletextpad=0.45)

        # ---- Panel B: outermost ZNGI with colored zones --------------------------
        R2 = np.linspace(0.01, 15, 2000)
        zngis = [zngi(R2, x) for x in x_vals]
        outermost = np.maximum.reduce(zngis)

        # Which prey "owns" each segment of the outermost envelope?
        owner = np.argmax(np.array(zngis), axis=0)  # index into x_vals

        # Intersection points between *consecutive* outermost prey
        # (sorted by R so that we walk left→right along the envelope)
        crossings = []  # (R*, P*, left_prey_idx, right_prey_idx)
        for i in range(zngi_number):
            ...
            for j in range(i + 1, zngi_number):
                ...
                if ps < 0:
                    continue
                # keep only if both i,j touch the outermost envelope at this point
                env_here = max(zngi(rs, xv) for xv in x_vals)
                if abs(ps - env_here) < 1e-6:
                    crossings.append((rs, ps, i, j))
        crossings.sort(key=lambda t: t[0])

        # Impact-vector slope for prey with trait x at point (rs, ps)
        def iv_slope(x, rs, ps):
            dR = g * x          # ∂(dR/dt)/∂n_i  evaluated at equilibrium (proportional)
            dP = e_p * f * x ** alpha   # ∂(dP/dt)/∂n_i
            # The impact vector points in direction (dR, -dP) in (R,P) space
            # but the convention in the ZNGI framework is  dR/dt < 0  and  dP/dt > 0
            # so the vector from the eq point toward the R-axis is (positive R, negative P)
            # slope = ΔP/ΔR :
            return -(dP / dp) / (dR / rs) if abs(dR) > 1e-14 else -1e6

        def iv_endpoint(rs, ps, slope, Rmax=15, Pmax=15):
            """Walk along the impact vector from (rs,ps) until hitting an axis or plot edge."""
            if abs(slope) < 1e-14:
                return (Rmax, ps)
            R_at_P0 = rs - ps / slope
            if 0 <= R_at_P0 <= Rmax and slope < 0:
                return (R_at_P0, 0.0)
            P_at_Rmax = ps + slope * (Rmax - rs)
            if 0 <= P_at_Rmax <= Pmax:
                return (Rmax, P_at_Rmax)
            P_at_R0 = ps + slope * (0 - rs)
            if 0 <= P_at_R0 <= Pmax:
                return (0, P_at_R0)
            return (Rmax, 0.0)

        # Build polygonal zones between consecutive crossings along the envelope.
        # The envelope goes from the left-most R* (most competitive prey, largest x)
        # rightward.  Between two crossings the envelope belongs to one prey;
        # the region *below* the envelope and between the two impact-vector lines
        # from the flanking crossings is the zone where that prey persists alone.
        # The triangular wedge between two impact vectors at a crossing is the
        # coexistence zone of the two prey that cross there.

        Rmax, Pmax = 15, 15

        # Pre-compute impact-vector endpoints at each crossing
        iv_ends = []
        for (rs, ps, li, ri) in crossings:
            sl_left  = iv_slope(x_vals[li], rs, ps)
            sl_right = iv_slope(x_vals[ri], rs, ps)
            ep_left  = iv_endpoint(rs, ps, sl_left)
            ep_right = iv_endpoint(rs, ps, sl_right)
            iv_ends.append((ep_left, ep_right))

        # --- Draw coexistence zones (hatched) at each crossing ---
        for k, (rs, ps, li, ri) in enumerate(crossings):
            ...
            # Build polygon: crossing → ep_left → along edges → ep_right → back
            coex_pts = [(rs, ps), ep_left]
            left_on_wall = ep_left[1] > 0.01   # hits right wall, not bottom
            right_on_wall = ep_right[1] > 0.01
            if left_on_wall and right_on_wall:
                coex_pts.append((Rmax, 0.0))
            elif left_on_wall:
                coex_pts.append((Rmax, 0.0))
                coex_pts.append((ep_right[0], 0.0))
            elif right_on_wall:
                coex_pts.append((ep_left[0], 0.0))
                coex_pts.append((Rmax, 0.0))
            else:
                coex_pts.append((ep_left[0], 0.0))
                coex_pts.append((ep_right[0], 0.0))
            coex_pts.append(ep_right)
            ax2.add_patch(plt.Polygon(np.array(coex_pts), closed=True, fc=coex_color,
                                      alpha=0.25, ec=coex_color, lw=1.2, hatch='///',
                                      zorder=0))

        # --- Draw single-prey zones (solid fill) along the envelope ---
        # Walk segments of the envelope owned by each prey between crossings
        # Leftmost segment: from R_star of most-competitive prey to first crossing
        # Rightmost segment: from last crossing rightward

        # All boundary R-values (sorted) including the R* of the first prey on envelope
        # and the plot edge
        seg_boundaries = []
        # start: R* of the leftmost owner on the envelope
        first_owner = owner[0]
        R_start = R_star(x_vals[first_owner])
        seg_boundaries.append(('start', R_start, 0.0, first_owner))
        for k, (rs, ps, li, ri) in enumerate(crossings):
            seg_boundaries.append(('cross', rs, ps, k))
        seg_boundaries.append(('end', Rmax, None, None))

        for s in range(len(seg_boundaries) - 1):
            sb = seg_boundaries[s]
            se = seg_boundaries[s + 1]
            ...
            if sb[0] == 'start':
                ...
                if se[0] == 'cross':
                    R_hi, P_hi = se[1], se[2]
                    # IV for the more-competitive prey (ri) at the next crossing
                    ep_hi = iv_ends[se[3]][1]
                else:
                    R_hi = Rmax
                    ep_hi = (Rmax, 0.0)
                    P_hi = zngi(R_hi, x_vals[prey_idx])
                mask = (R2 >= R_lo) & (R2 <= R_hi)
                Rseg = R2[mask]
                Pseg = np.array([zngi(r, x_vals[prey_idx]) for r in Rseg])
                Pseg = np.clip(Pseg, 0, Pmax)
                poly = np.column_stack((
                    np.concatenate(([R_lo], Rseg, [ep_hi[0]])),
                    np.concatenate(([0.0],  Pseg, [ep_hi[1]]))
                ))
                ax2.add_patch(plt.Polygon(poly, closed=True,
                                          fc=prey_colors_all[prey_idx], alpha=0.18,
                                          ec='none', zorder=0))

            elif sb[0] == 'cross':
                k_lo = sb[3]
                rs_lo, ps_lo = sb[1], sb[2]
                _, _, li_lo, _ = crossings[k_lo]
                prey_idx = li_lo  # prey to the RIGHT of crossing has LOWER index (more defended)
                ep_lo = iv_ends[k_lo][0]  # IV for li (more-defended) species at left crossing

                if se[0] == 'cross':
                    k_hi = se[3]
                    rs_hi, ps_hi = se[1], se[2]
                    ep_hi = iv_ends[k_hi][1]  # IV for ri (more-competitive) species at right crossing
                else:
                    rs_hi = Rmax
                    ep_hi = (Rmax, 0.0)
                ...
                # polygon: envelope curve on top, IV lines / axes on bottom
                poly_pts = [(rs_lo, ps_lo)]
                for r, p in zip(Rseg, Pseg):
                    poly_pts.append((r, p))
                if se[0] == 'cross':
                    poly_pts.append((rs_hi, ps_hi))
                poly_pts.append(ep_hi)
                # close along axes / edges back to ep_lo
                if ep_hi[1] > 0.01 and ep_lo[1] > 0.01:
                    # both IVs hit side walls — close along top-right corner
                    poly_pts.append((Rmax, 0.0))
                elif ep_hi[1] > 0.01:
                    # right IV hits side wall, left IV hits bottom
                    poly_pts.append((Rmax, 0.0))
                elif ep_lo[1] > 0.01:
                    # left IV hits side wall, right IV hits bottom
                    # (unlikely for this model but handle it)
                    pass
                # else: both hit bottom axis — R-axis connects them
                poly_pts.append(ep_lo)

                ax2.add_patch(plt.Polygon(np.array(poly_pts), closed=True,
                                          fc=prey_colors_all[prey_idx], alpha=0.18,
                                          ec='none', zorder=0))

        # --- Draw outermost envelope curves (colored per prey) ---
        for i, z in enumerate(zngis):
            mask = np.array(z) >= outermost - 1e-8
            idx_arr = np.where(mask)[0]
            if len(idx_arr) == 0:
                continue
            # widen mask slightly for continuity
            mask[max(idx_arr[0] - 1, 0)] = True
            mask[min(idx_arr[-1] + 1, len(mask) - 1)] = True
            ax2.plot(R2[mask], np.array(z)[mask],
                     color=prey_colors_all[i], linewidth=line_lw, zorder=3)

        # --- Draw impact vectors at crossings ---
        for k, (rs, ps, li, ri) in enumerate(crossings):
            ax2.scatter(rs, ps, color='red', s=50, zorder=6, edgecolor='white', linewidth=0.8)
            ep_left, ep_right = iv_ends[k]
            for ep_pt, pidx in [(ep_left, li), (ep_right, ri)]:
                ax2.annotate('', xy=(rs, ps), xytext=ep_pt,
                             arrowprops=dict(arrowstyle='-|>',
                                            color=prey_colors_all[pidx],
                                            lw=impact_lw, mutation_scale=18,
                                            shrinkA=0, shrinkB=6, alpha=0.7),
                             zorder=4)

        # # --- Labels for single-prey and coexistence zones ---
        # lbox = dict(boxstyle='round,pad=0.2', fc='white', ec='none', alpha=0.78)
        # # Place labels roughly in the center of each zone
        # if len(crossings) >= 3:
        #     # Prey A alone (leftmost, most competitive)
        #     ax2.text(2.0, 1.0, 'Prey A\nonly', fontsize=13, color=prey_colors_all[3],
        #              fontweight='bold', ha='center', bbox=lbox, zorder=5)
        #     # Coexistence A + B
        #     ax2.text(2.6, 0.5, 'A + B',
        #              fontsize=12, color='black', fontweight='bold', ha='center',
        #              bbox=lbox, zorder=5)
        #     # Prey B alone
        #     ax2.text((crossings[1][0] + crossings[2][0]) / 2, 1.5, 'Prey B\nonly',
        #              fontsize=13, color=prey_colors_all[2], fontweight='bold',
        #              ha='center', bbox=lbox, zorder=5)
        #     # Coexistence B + C
        #     ax2.text(7.0, 0.5, 'B + C',
        #              fontsize=12, color='black', fontweight='bold', ha='center',
        #              bbox=lbox, zorder=5)
        #     if len(crossings) >= 3:
        #         # Prey C alone
        #         ax2.text(crossings[2][0] + 2.0, 3.0, 'Prey C\nonly',
        #                  fontsize=13, color=prey_colors_all[1], fontweight='bold',
        #                  ha='center', bbox=lbox, zorder=5)
        #         # Coexistence C + D
        #         ax2.text(13.0, 0.5, 'C + D',
        #                  fontsize=12, color='black', fontweight='bold', ha='center',
        #                  bbox=lbox, zorder=5)
        #         # Prey D alone (rightmost, most defended)
        #         ax2.text(13.0, 5.0, 'Prey D\nonly', fontsize=13,
        #                  color=prey_colors_all[0], fontweight='bold', ha='center',
        #                  bbox=lbox, zorder=5)
...
def _(np, plt, resolve_path, save_fig):
    def plot_pip():
        ...
        for idx, (ax, alpha) in enumerate(zip(axes, [0.5, 1, 2])):
            ...
            ax.set_ylabel('Mutant trait', fontsize=24)
            ax.tick_params(axis='both', which='major', labelsize=20)
            desc = {0.5: 'strong trade-off', 1: 'linear trade-off', 2: 'weak trade-off'}.get(alpha, '')
            ax.set_title(r'$\alpha$ = ' + f'{alpha}' + (f' ({desc})' if desc else ''), fontsize=22)
